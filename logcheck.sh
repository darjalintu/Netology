...
if tail -n 36 sysinfo.log | awk '($6 / $8) >= 0.6 {alertfound = 1; exit} END {exit !alertfound}'; then
  echo "Зафиксировано в течении последних 3 минут, что использовано > 60% объема оперативной памяти"
  alert=1
fi

if tail -n 60 sysinfo.log | awk '($10 / $12) >= 0.6 {alertfound = 1; exit} END {exit !alertfound}'; then
